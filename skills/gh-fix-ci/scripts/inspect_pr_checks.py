...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "GitHub PR failing check를 확인하고, GitHub Actions log를 가져와 실패 snippet을 추출합니다."
        ),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        add_help=False,
    )
    parser.add_argument("-h", "--help", action="help", help="도움말을 출력하고 종료합니다.")
    parser.add_argument("--repo", default=".", help="대상 Git repository 안의 path.")
    parser.add_argument(
        "--pr", default=None, help="PR 번호 또는 URL. 생략하면 현재 branch PR을 사용합니다."
    )
    parser.add_argument("--max-lines", type=int, default=DEFAULT_MAX_LINES, help="출력할 최대 log line 수.")
    parser.add_argument("--context", type=int, default=DEFAULT_CONTEXT_LINES, help="실패 marker 주변에 포함할 context line 수.")
    parser.add_argument("--json", action="store_true", help="text 대신 JSON을 출력합니다.")
...
def main() -> int:
    ...
    if repo_root is None:
        print("Error: Git repository 내부가 아닙니다.", file=sys.stderr)
        return 1
    ...
    if not failing:
        print(f"PR #{pr_value}: failing check가 없습니다.")
        return 0
...
def ensure_gh_available(repo_root: Path) -> bool:
    if which("gh") is None:
        print("Error: gh가 설치되어 있지 않거나 PATH에 없습니다.", file=sys.stderr)
        return False
    result = run_gh_command(["auth", "status"], cwd=repo_root)
    if result.returncode == 0:
        return True
    message = (result.stderr or result.stdout or "").strip()
    print(message or "Error: gh 인증이 되어 있지 않습니다.", file=sys.stderr)
    return False
...
def resolve_pr(pr_value: str | None, repo_root: Path) -> str | None:
    ...
    if result.returncode != 0:
        message = (result.stderr or result.stdout or "").strip()
        print(message or "Error: PR을 확인할 수 없습니다.", file=sys.stderr)
        return None
    try:
        data = json.loads(result.stdout or "{}")
    except json.JSONDecodeError:
        print("Error: PR JSON을 parse할 수 없습니다.", file=sys.stderr)
        return None
    number = data.get("number")
    if not number:
        print("Error: PR 번호를 찾을 수 없습니다.", file=sys.stderr)
        return None
    return str(number)
...
def fetch_checks(pr_value: str, repo_root: Path) -> list[dict[str, Any]] | None:
    ...
    if result.returncode != 0:
        ...
        if available_fields:
            ...
            if not selected_fields:
                print("Error: gh pr checks에서 사용할 수 있는 field가 없습니다.", file=sys.stderr)
                return None
            result = run_gh_command(
            ...
            if result.returncode != 0:
                message = (result.stderr or result.stdout or "").strip()
                print(message or "Error: gh pr checks가 실패했습니다.", file=sys.stderr)
                return None
        else:
            print(message or "Error: gh pr checks가 실패했습니다.", file=sys.stderr)
            return None
    try:
        ...
        print("Error: checks JSON을 parse할 수 없습니다.", file=sys.stderr)
        return None
    if not isinstance(data, list):
        print("Error: 예상하지 못한 checks JSON 형식입니다.", file=sys.stderr)
        return None
    return data
...
def analyze_check(
    ...
    if run_id is None:
        ...
        base["note"] = "detailsUrl에서 GitHub Actions run id를 찾지 못했습니다."
        return base
    ...
    if log_status == "pending":
        ...
        base["note"] = log_error or "아직 log를 사용할 수 없습니다."
        if metadata:
            base["run"] = metadata
        return base
...
def fetch_run_log(run_id: str, repo_root: Path) -> tuple[str, str]:
    ...
    if result.returncode != 0:
        ...
        return "", error or "gh run view가 실패했습니다."
...
def fetch_job_log(job_id: str, repo_root: Path) -> tuple[str, str]:
    ...
    if not repo_slug:
        return "", "Error: job log를 가져올 repository 이름을 확인할 수 없습니다."
    ...
    if returncode != 0:
        ...
        return "", message or "gh api job logs가 실패했습니다."
    if is_zip_payload(stdout_bytes):
        return "", "Job log가 zip archive로 반환되어 parse할 수 없습니다."
...
def render_results(pr_number: str, results: Iterable[dict[str, Any]]) -> None:
    ...
    print(f"PR #{pr_number}: failing check {len(results_list)}개를 분석했습니다.")
    for result in results_list:
        print("-" * 60)
        print(f"Check: {result.get('name', '')}")
        if result.get("detailsUrl"):
            print(f"상세 URL: {result['detailsUrl']}")
