...
원격 read 전에 `gh auth status`를 실행합니다. 인증이 실패하더라도 `API rate limit exceeded`, `X-RateLimit-Remaining: 0`, REST `/user` 403이면 `gh auth refresh`나 device-code 로그인을 반복하지 말고 Codex GitHub connector fallback을 먼저 사용합니다.
...
2. thread-aware read로 review context를 확인합니다.
   ...
   - `gh` REST rate limit 신호가 나오면 Codex GitHub connector를 로드하고 `_fetch_pr`, `_list_pull_request_review_threads`, `_list_pull_request_reviews`, `_fetch_pr_comments` 계열로 같은 맥락을 수집합니다.
...
7. GitHub write를 dry-run 후 apply합니다.
   ...
   - `gh` write가 REST rate limit에 막히고 connector가 해당 작업을 지원하면 `_reply_to_review_comment`, `_resolve_review_thread`, `_add_comment_to_issue` 같은 Codex GitHub connector 도구로 적용합니다. connector parameter가 부족하거나 도구가 없으면 thread를 unresolved로 남기고 reset time/capability gap을 보고합니다.
8. 결과를 요약합니다.
...
- 중간에 실제 auth 문제가 발생하면 사용자에게 재인증 후 retry를 요청합니다.
- 중간에 `gh` REST rate limit 문제가 발생하면 사용자에게 `gh auth refresh`를 반복시키지 말고 Codex GitHub connector fallback으로 가능한 read/write를 먼저 시도합니다.

## Fallback

`gh`로 PR을 명확히 확인할 수 없으면 blocker가 repository scope 부족인지, PR context 부족인지, CLI authentication 문제인지, REST rate limit인지 분리합니다.

- REST rate limit이면 `tool_search`로 GitHub connector를 로드하고 가벼운 read-only 호출로 인증을 확인합니다.
- connector가 지원하는 PR/issue/review/comment/thread 작업은 connector로 진행합니다.
- connector가 지원하지 않는 작업은 `gh` reset time 또는 필요한 repo/PR 식별자를 보고합니다.
